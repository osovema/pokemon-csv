...
class Pokedex:
    ...
    def remove_pokemon(self, pokedex_number):
        self.pokemons = [pokemon for pokemon in self.pokemons if pokemon.pokedex_number != pokedex_number]
        with open('data/pokemon.csv', 'r', encoding='utf-8') as csvfile:
            lines = csvfile.readlines()
        with open('data/pokemon.csv', 'w', encoding='utf-8') as csvfile:
            for line in lines:
                if not line.startswith(f"{pokedex_number},"):
                    csvfile.write(line)
